#!/usr/bin/env bash
# Task 20 — v1 -> v2 update test against a running, installed VM.
#
# Assumes a VM installed from the v1 image (see tests/vm/run.sh --image ...:v1), boots it,
# then `bootc switch`-es to the v2 image, reboots, and verifies the new version is booted.
#
# Usage: tests/vm/upgrade.sh --state FILE [--to REF] [--from-version 1] [--to-version 2]
set -euo pipefail
...
to="${to:-192.168.122.1:5000/cachyos-bootc:v${to_version}}"
...
after="$(ssh_guest "${vm}" 'cat /usr/lib/bootc-image-version' 2>/dev/null | tr -d '[:space:]')"
...
if ssh_guest "${vm}" 'bootc status --json' 2>/dev/null | grep -q '"staged": *null'; then
